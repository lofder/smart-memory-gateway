...
2. ~~`mem0_get_all` 缺少权限检查~~ ✅ v2.0.0 已修复（scope 过滤 + 权限校验）
3. ~~巩固分组用词桶启发式，非向量相似度~~ ✅ v2.0.0 已修复（关键词预过滤 + Union-Find + max_groups 限制）
4. ~~weekly 衰减用 dedup 后的旧快照，未在冲突/巩固后刷新~~ ✅ v2.0.0 已修复（冲突/巩固后刷新快照再执行衰减）
5. 分类器双层设计完整但维护管线未接入
