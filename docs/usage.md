...
mcp(action="call", server="engram", tool="mem0_search", args={
...
# Add a memory / 存储记忆
mcp(action="call", server="engram", tool="mem0_add", args={
...
# Health check / 健康检查
mcp(action="call", server="engram", tool="mem0_status", args={})

# Get all memories / 获取全部记忆
mcp(action="call", server="engram", tool="mem0_get_all", args={
    "scope": "global"                # Optional scope filter / 可选 scope 过滤
})

# Run maintenance / 运行维护
mcp(action="call", server="engram", tool="mem0_maintenance", args={
